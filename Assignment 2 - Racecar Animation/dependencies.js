...
class Traffic_Cone extends Shape {
  constructor() {
    super();

    //Overall Vertices # 0, 1, 2, 3
    var bottom_positions = [Vec.of(-0.5, 0, -0.5), Vec.of(-0.5, 0, 2.5), Vec.of(2.5, 0, -0.5), Vec.of(2.5, 0, 2.5)];
    var bottom_normals = [Vec.of(0, -1, 0), Vec.of(0, -1, 0), Vec.of(0, -1, 0), Vec.of(0, -1, 0)];
    var bottom_textures = [Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0)];
    var bottom_indices = [0, 1, 2, 3, 1, 2];

    //Overall Vertices # 4, 5, 6, 7, 8, 9
    var lr_positions = [Vec.of(0, 0, 0), Vec.of(0, 0, 2), Vec.of(1, 3, 1), Vec.of(2, 0, 0), Vec.of(2, 0, 2), Vec.of(1, 3, 1)];
    var lr_normals = [Vec.of(-1, 0, 0), Vec.of(-1, 0, 0), Vec.of(-1, 0, 0), Vec.of(1, 0, 0), Vec.of(1, 0, 0), Vec.of(1, 0, 0 )];
    var lr_textures = [Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0)];
    var lr_indices = [4, 5, 6, 7, 8, 9]

    //Overall Vertices # 10, 11, 12, 13, 14, 15, 16
    var fb_positions = [Vec.of(2, 0, 2), Vec.of(0, 0, 2), Vec.of(1, 3, 1), Vec.of(0, 0, 0), Vec.of(2, 0, 0), Vec.of(1, 3, 1)];
    var fb_normals = [Vec.of(0, 0, 1), Vec.of(0, 0, 1), Vec.of(0, 0, 1), Vec.of(0, 0, -1), Vec.of(0, 0, -1), Vec.of(0, 0, -1)];
    var fb_textures = [Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0), Vec.of(0, 0)];
    var fb_indices = [10, 11, 12, 13, 14, 15
    ]

    this.positions.push(...bottom_positions, ...lr_positions, ...fb_positions);
    this.normals.push(...bottom_normals, ...lr_normals, ...fb_normals);
    this.texture_coords.push(...bottom_textures, ...lr_textures, ...fb_textures);
    this.indices.push(...bottom_indices, ...lr_indices, ...fb_indices);
  }
}

// *********** TRIANGLE ***********
// First, the simplest possible Shape – one triangle.  It has 3 vertices, each
...
// *********** SQUARE ***********
// A square, demonstrating shared vertices.  On any planar surface, the interior edges don't make any important seams.
